/**
 * Claude tool definition for the 'analyze_idea' function.
 *
 * This tool definition enforces structured JSON output from Claude via tool use.
 * The input schema mirrors ClaudeAnalysisOutputSchema (Zod schema in schemas.ts).
 *
 * Ref: design/components.md — PromptBuilder (tool schema)
 *      design/integration.md — Claude API (tool use pattern)
 *
 * Task 2.2
 */

/** Claude tool definition object — passed in the `tools` array of messages.create() */
export const ANALYSIS_TOOL_DEFINITION = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      portfolio_matches: {
        type: "array",
        description: "Relevance of this idea to each AppliCAD product. Include ALL 4 products.",
        items: {
          type: "object",
          ...
          properties: {
            product: {
              ...
              description: "AppliCAD product name.",
            },
            relevance: {
              type: "string",
...
/** Type-safe tool_choice parameter for Claude messages.create() */
